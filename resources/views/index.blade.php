...
<head>
    ...
    <script src="https://unpkg.com/string-similarity@4.0.2/umd/string-similarity.min.js" referrerpolicy="no-referrer">
    </script>
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/sql-formatter/3.1.0/sql-formatter.min.js" crossorigin="anonymous"
...
<body class="bg-gray-100 tracking-wide bg-gray-200">
    ...
    <div id="app" v-cloak class="w-full flex lg:pt-10">
        <aside
            class="text-sm ml-1.5 text-grey-darkest break-all bg-gray-200 pl-2 h-screen sticky top-1 overflow-auto w-10/12">
            <section class="pt-5 pl-2 pr-2 pb-5 border mb-10 rounded bg-white shadow">
                <div class="font-sans">
                    ...
                    <input type="text" v-model="filterTerm" @input="filterDocs"
                        class="w-full p-2 border-2 border-gray-300 rounded" placeholder="search route or description">
                </div>
            </section>
            <section class="pt-5 pl-2 pr-2 pb-5 border mb-10 rounded bg-white shadow">
                <div class="font-sans">
                    ...
                    <p class="text-xs pb-2 font-medium text-gray-500">Default headers sent on every request. Format
                        ...
                        <tr v-if="!docs[{{ $index }}]['isHidden']">
                            <td>
                                <a href="#{{ $doc['methods'][0] . '-' . $doc['uri'] }}"
                                    ...
                                    <span class="text-xs"
                                        v-bind:class="docs[{{ $index }}]['isActiveSidebar'] ? 'font-bold':''">
                                        ...
                                        <span
                                            class="font-bold text-red-600 border rounded-full pr-1 pl-1 border-red-500"
        ...
        <div class="ml-6 mr-6 pl-2 w-2/3 p-2" style="width: 100%">
            ...
            <br>
            @foreach ($docs as $index => $doc)
                <section class="pt-5 pl-2 pr-2 pb-5 border-2 mb-10 rounded bg-white shadow"
                    v-if="!docs[{{ $index }}]['isHidden']">
                    <div class="border-2 rounded" id="{{ $doc['httpMethod'] . '-' . $doc['uri'] }}">
                        <h1
                            class="text-sm break-normal text-black bg-indigo-50 break-normal font-sans pb-1 pt-1 text-black">
                            <span
                                class="w-20
                        font-medium
                        inline-flex
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="align-left pl-2 pr-2 break-all">
                                        <a target="_blank"
                                            href="@{{ window.location.origin }}/{{ $doc['uri'] }}">@{{ window.location.origin }}/{{ $doc['uri'] }}</a>
                                    </td>
                                </tr>
                                <tr>
                                ...
                                @foreach ($doc['middlewares'] as $middleware)
                                    <tr>
                                        <td class="align-left pl-2 pr-2 bg-gray-100 border-r-2">Middleware
                                            {{ $loop->index + 1 }}</td>
                        ...
                        <div class="border-2 mr-4 mt-4 p-4 rounded">
                            ...
                            <table class="table-fixed align-left text-sm mt-3">
                                <thead class="border">
                                    <tr class="border">
                                        <th
                                            class="border border-gray-300 pl-2 pr-16 pt-1 pb-1 w-min text-left bg-gray-200">
                                            No.</th>
                                        <th
                                            class="border border-gray-300 pl-2 pr-16 pt-1 pb-1 w-min text-left bg-gray-200">
                                            Attributes</th>
                                            ...
                                            <td class="border pl-3 pt-1 pb-1 pr-2">
                                                @foreach ($rules as $rule)
                                                    ...
                                                    @if (str_contains($rule, 'numeric'))
                                                        <span
                                                            class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-blue-800 bg-blue-300 rounded">Numeric</span>
                                                    @endif
                                                    ...
                                                    @if (str_contains($rule, 'json'))
                                                        <span
                                                            class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-green-100 bg-green-500 rounded">JSON</span>
                                                    @endif
                    ...
                    <button v-if="docs[{{ $index }}]['try']" @click="request(docs[{{ $index }}])"
                    ...
                    <div class="grid grid-cols-1 mt-3 pr-2 overflow-auto">
                        <div class="">
                            <div v-if="docs[{{ $index }}]['try']">
                                <h3 class="font-medium">REQUEST URL<sup class="text-red-500 font-bold">
                                        *required</sup></h3>
                                ...
                                @if (!in_array('GET', $doc['methods']))
                                    <h3 class="font-medium">REQUEST BODY<sup class="text-red-500"> *required</sup>
                                    </h3>
                                    ...
                                    <prism-editor class="my-prism-editor"
                                        ...
                                        v-model="docs[{{ $index }}]['body']" :highlight="highlighter"
                                        line-numbers></prism-editor>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2"
                            ...
                            <div class="border-r-2">
                                ...
                                <table class="table-fixed text-sm mt-5">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="align-left pl-2 pr-2 break-all">
                                                <span v-if="docs[{{ $index }}]['responseOk']"
                                        ...
                                        <tr>
                                            ...
                                            <td class="align-left pl-2 pr-2 break-all">
                                                <span v-if="docs[{{ $index }}]['responseTime']"
                                ...
                                <prism-editor v-if="docs[{{ $index }}]['responseHeaders']"
                                    ...
                                    readonly v-model="docs[{{ $index }}]['responseHeaders']"
                                    :highlight="highlighterAtom" line-numbers></prism-editor>
                            </div>
                            <div>
                                <p class="text-xs pb-2 font-medium text-gray-800">Response from the server</p>
                                <prism-editor v-if="docs[{{ $index }}]['response']"
                                    class="my-prism-editor shadow-inner border-gray-400 border-2 rounded"
                                    style="min-height:100px;height:300px;background:#2d2d2d;color: #ccc;resize:both"
                                    readonly v-model="docs[{{ $index }}]['response']" :highlight="highlighter"
                                    line-numbers></prism-editor>
                            </div>
                            <div class="border-r-2">
                                ...
                                <table class="table-fixed text-sm mt-5">
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="align-left pl-2 pr-2 bg-gray-100 border-r-2">Total Query time
                                            </td>
                                            <td class="align-left pl-2 pr-2 break-all">
                                                <span
                                                    class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-gray-800 bg-yellow-400 rounded">
                            ...
                            <div class="">
                                ...
                                <div v-for="(query, index) in docs[{{ $index }}]['queries']">
                                    ...
                                    <prism-editor v-model="sqlFormatter.format(query['sql'])" class="my-prism-editor"
                                        style="padding:10px 0px 10px 0px;min-height:20px;height:350px;background:rgb(52 33 33);color: #ccc;resize:both;"
                                        readonly :highlight="highlighter" line-numbers></prism-editor>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        var guessValue = function(attribute, rules) {
            ...
            if (validations.isInteger) {
                validations.value = Math.floor(Math.random() * (validations.max - validations.min + 1) + validations
                    .min)
            }
            ...
            methods: {
                ...
                filterDocs() {
                    for (doc of this.docs) {
                        doc['isHidden'] = (!doc['docBlock'].toLowerCase().includes(this.filterTerm.toLowerCase()) &&
                            !doc['uri'].includes(this.filterTerm))
                    }
                },
                request(doc) {
